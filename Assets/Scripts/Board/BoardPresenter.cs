...
public class BoardPresenter : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
                MergeAction merge = MergeInvoker.UndoMerge(BoardModel);
                if(merge.Plan != null)
                    StartCoroutine(AnimateTurnSequenceReversed(merge.Plan));
        }
    }
    ...
    private void HandleBlobSelected(Blob selectedBlob)
    {
        ...
        else
        {
            ...
            MergeInvoker.ExecuteMerge(plan, BoardModel);
    ...
    private IEnumerator AnimateInitialBlobs()
    {
        ...
        {
            StartCoroutine(SpawnBlob(view.Model));
    ...
    private IEnumerator AnimateTurnSequenceReversed(MergePlan plan)
    {
        if (plan.DeferredPlan != null)
        {
            yield return AnimateMergeFromPlan(-plan.DeferredPlan);
    ...
    public IEnumerator AnimateMergeFromPlan(MergePlan plan)
    {
        ...
        while (currentPos != endPosition + direction)
        {
            ...
            Blob blobToRemoveOnPath = plan.BlobsToRemoveOnPath.FirstOrDefault(b => b.Value == currentPos).Key;
            Blob blobToCreateOnPath = plan.BlobsToCreateOnPath.FirstOrDefault(b => b.Value == currentPos).Key;


            yield return MoveBlob(plan.SourceBlob, currentPos, moveDuration, ease);

            if (blobToRemoveOnPath != null)
            {
               yield return StartCoroutine(RemoveBlob(blobToRemoveOnPath));
            }

            if (blobToCreateOnPath != null)
            {
               yield return StartCoroutine(SpawnBlob(blobToCreateOnPath));
            }
            currentPos += direction;
        }
        StartCoroutine(AnimateScale(plan.SourceBlob));

        foreach (Blob blob in plan.BlobsToRemoveAfterMerge)
        {
           yield return StartCoroutine(RemoveBlob(blob));
        }
        foreach(Blob blob in plan.BlobsToCreateAfterMerge)
        {
          yield return StartCoroutine(SpawnBlob(blob));
        }
        plan.OnMergeComplete?.Invoke(plan);
    }

    private IEnumerator AnimateScale(Blob blob)
    {
        if (GetBlobView(blob.ID) is { } view )
        {

            Vector2 scale = view.GetScaleFromBlobSize();
            yield return view.transform.DOScale(scale, scaleDuration).WaitForCompletion();
    ...
    public IEnumerator AnimateEndTurnSequence()
    {
        ...
        var rest = BoardModel.GetAllBlobs();
        foreach (Blob blob in rest)
        {
            yield return RemoveBlob(blob);
        }
    ...
    public IEnumerator MoveBlob(Blob blob, Vector2Int toPosition, float duration = 0.5f, Ease ease = Ease.Linear)
    {
    ...
    public IEnumerator RemoveBlob(Blob blob)
    {
        if (GetBlobView(blob.ID) is { } view)
        {
    ...
    public IEnumerator SpawnBlob(Blob blob)
    {
        if (GetBlobView(blob.ID) is { } view)
        {
