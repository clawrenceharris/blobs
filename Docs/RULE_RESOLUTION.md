# Blobs — resolution rules

**Decision date: September 9, 2026.** Detailed authority for move ordering and edge cases. Read [game design](GAME_DESIGN.md) for the player-facing overview. These are intended rules, not implementation status.
...
See [Undo and Restart behavior](UNDO_AND_RESTART.md) for history, playback, and command availability. See [level authoring](LEVEL_AUTHORING.md) for structural validation.
