...
- [ ] **1. Resolution outcomes and stopping.** Update `Scripts/Core/Resolution/MoveResolver.cs`, `CollisionPlan.cs`, and `Scripts/Core/Strategy/MoveStrategy.cs` / `MergeStrategy.cs` to distinguish rejection, valid unchanged contact, and board-changing success. Intermediate Rocks must terminate safely. Validate terrain only until the reached terminator. Keep all failures atomic.
