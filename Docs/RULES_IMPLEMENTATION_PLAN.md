...
- [ ] **3. Flag contract.** Require Normal source and matching color; explicitly selected Flag must actually be captured and leave zero clearable blobs after the whole simulation. Reject intermediate Flags. Preserve earlier chain-clears in simulation when deciding capture eligibility. Implement agreed reason precedence in result/feedback mapping. AWmonth3
- [ ] **4. Ghost/Grave edge cases.** Update `GhostMergeStrategy` and `GhostReturnEffect` for a Ghost starting haunt on a Grave, including a zero-distance clearing event. Preserve intermediate occupants and restore consumed landing occupants during Undo.
