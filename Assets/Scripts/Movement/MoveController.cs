...
public class MoveController : MonoBehaviour
{
    ...
    private void InvokeMove(Move move){
        Blob selectedBlob = board.GetBlobAt(move.start);
        ...
        //invoke the merge action passing in the blob to move to
            ...
            if (selectedBlob && submoves.Count > 0 && IsLegalMove(submoves))
            {
    ...
    private bool IsLegalMove(List<Move> submoves){
        Blob selectedBlob = board.GetBlobAt(submoves[0].start);//get the first blob selected
        ...
        foreach(Move submove in submoves){
            ...
            Blob  blobToMoveTo = board.GetBlobAt(submove.end);
